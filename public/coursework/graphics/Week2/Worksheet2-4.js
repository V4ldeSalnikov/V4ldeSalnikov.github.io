...
async function main() {
  ...
  const btnCirc = document.getElementById('mode-circle');
  ...
  const setMode = (m) => {
    ...
    btnCirc.setAttribute('aria-pressed',  m === 'circle' ? 'true':'false');
    if (triClicks.length) { positions.splice(-6*triClicks.length, 6*triClicks.length); colors.splice(-6*triClicks.length, 6*triClicks.length); triClicks.length = 0; upload(); render(); }
    if (circCenter)       { positions.splice(-6, 6); colors.splice(-6, 6); circCenter = null; upload(); render(); }
  };
  ...
  function addCircle(posArr, colArr, center, radius, innerColor, outerColor, segments=64) {
    const [cx, cy] = center;
    ...
    for (let i = 0; i < segments; ++i) {
      ...
      const p0 = vec2(cx, cy);
      const p1 = vec2(cx + Math.cos(a0) * radius, cy + Math.sin(a0) * radius);
      const p2 = vec2(cx + Math.cos(a1) * radius, cy + Math.sin(a1) * radius);
      posArr.push(p0, p1, p2);
      colArr.push(vec3(innerColor[0],innerColor[1],innerColor[2]),
                  vec3(outerColor[0],outerColor[1],outerColor[2]),
                  vec3(outerColor[0],outerColor[1],outerColor[2]));
    }
  }
  ...
  canvas.addEventListener('click', (e) => {
    ...
    if (mode === 'circle') {
      if (!circCenter) {
        addPoint(positions, colors, [x,y], pointSize, currentVertexColor);
        circCenter = { p:[x,y], color:[...currentVertexColor] };
        upload(); render();
      } else {
        positions.splice(-6, 6);
        colors.splice(-6, 6);

        const center = circCenter.p;
        const dx = x - center[0], dy = y - center[1];
        const radius = Math.hypot(dx, dy);

        addCircle(positions, colors, center, radius, COLORS.white, currentVertexColor, 64);
