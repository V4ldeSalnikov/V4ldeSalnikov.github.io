...
async function main() {
  ...
  const shadow = setupShadow(device, shadowPipeline, teapot);
  ...
  function frame(timestampMs) {
    ...
    data.set(lightPos, 32);
    data.set([eye[0], eye[1], eye[2], 1.0], 36);
    device.queue.writeBuffer(teapot.uniformBuffer, 0, data);

    const Ms = shadowMatrix(lightPos, 1.0 + 0.003);
    const shadowMVP = mult(viewProj, mult(Ms, model));
    device.queue.writeBuffer(shadow.uniformBuffer, 0, flatten(shadowMVP));
...
function createShadowPipeline(device, format) {
  const module = device.createShaderModule({ code: document.getElementById("shadow-wgsl").text });
  return device.createRenderPipeline({
    layout: "auto",
    vertex: {
      module, entryPoint: "main_vs",
...
async function setupTeapot(device, pipeline, status) {
  ...
  return { pipeline, vertexBuffer: mesh.vertexBuffer, indexBuffer: mesh.indexBuffer, indexCount: mesh.indexCount, uniformBuffer, bindGroup, mesh };
}

function setupShadow(device, pipeline, teapot) {
  const uniformBuffer = device.createBuffer({ size: 64, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
  const bindGroup = device.createBindGroup({
    layout: pipeline.getBindGroupLayout(0),
    entries: [{ binding: 0, resource: { buffer: uniformBuffer } }]
  ...
  return {
    pipeline,
    vertexBuffer: teapot.vertexBuffer,
    indexBuffer: teapot.indexBuffer,
