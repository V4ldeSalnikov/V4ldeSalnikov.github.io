...
async function main() {
  ...
  const normalMap   = await loadTexture(device, "normalmap.png");
  ...
  const sphereU = makeObject(device, pipeline, sampler, cubeTexture, normalMap);
  const quadU   = makeObject(device, pipeline, sampler, cubeTexture, normalMap);
  ...
  function draw(seconds) {
    ...
    setUniforms(device, sphereU, mult(Mst, mult(proj, view)), I, eye, 1.0);
    setUniforms(device, quadU, I, mult(viewRotationInverse(view), inverse(proj)), eye, 0.0);
...
function makeObject(device, pipeline, sampler, cubeTexture, normalMap) {
  ...
  const bindGroup = device.createBindGroup({
    ...
    entries: [
      ...
      { binding: 3, resource: normalMap.createView() }
    ]
  });
  return { buffer, bindGroup };
}

function setUniforms(device, obj, mvp, mtex, eye, reflective) {
  const data = new Float32Array(40);
  data.set(flatten(mvp), 0);
  data.set(flatten(mtex), 16);
  data.set([eye[0], eye[1], eye[2], 1.0], 32);
  data[36] = reflective;
