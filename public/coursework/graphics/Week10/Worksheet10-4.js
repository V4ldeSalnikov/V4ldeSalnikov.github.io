...
function createTrackball(canvas) {
  const tb = {
    ...
    panX: 0.0, panY: 0.0,
    mode: "orbit",
    rolling: false, spinning: false, moved: false,
    last: null, lastX: 0, lastY: 0, lastRoll: 0
  };
  ...
  tb.update = () => {
    if (tb.rolling && tb.moved) { tb.q_rot.multiply(tb.q_inc); tb.moved = false; }
    else if (tb.spinning)       { tb.q_rot.multiply(tb.q_inc); }
  };
  ...
  function onUp() {
    tb.rolling = false;
    if (tb.mode === "orbit" && tb.moved && (Date.now() - tb.lastRoll) < SPIN_MS) {
      tb.spinning = true;
    } else {
      tb.spinning = false;
      tb.q_inc.setIdentity();
    }
  }
  ...
  for (const [mode, id] of Object.entries(buttons)) {
    document.getElementById(id).addEventListener("click", () => {
      tb.mode = mode;
      if (mode !== "orbit") { tb.spinning = false; tb.q_inc.setIdentity(); }
